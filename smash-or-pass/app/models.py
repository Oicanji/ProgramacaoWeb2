...
"""
LIST ANSWERS - Lista de respostas dos questionários
É responsável por armazenar as listas de respostas dos usuários (Answers)
Suas relações são com a tabela de Quiz

Ele possui ainda uma lista de respostas do quiz, que é uma relação com a tabela de respostas (Answers)
"""
class List_Answers(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
...
É responsansável por linkar as (Answer) de respostas dos usuários

Ele possui ainda uma lista de respostas das perguntas (question), que é uma relação com a tabela de respostas (Answer)
"""
class Answers(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    id_list_answers = db.Column(db.Integer, db.ForeignKey('list_answers.id'))
